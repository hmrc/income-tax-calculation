...
class CalculationDetailControllerSpec extends TestSuite {
  ...
  "CalculationDetailController.calculationDetailByCalcId" should {
    ...
    "return a success response with a calculation model" in {
      ...
      mockAuth()
      calculationResponseByCalcId(nino, calculationId, Some(taxYear), None)(Right(successModelFull))
    ...
    "return a 500 error response with a calculation error" in {
      ...
      mockAuth()
      calculationResponseByCalcId(nino, calculationId, Some(taxYear), None)(Left(ErrorModel(INTERNAL_SERVER_ERROR, ErrorBodyModel("INTERNAL_SERVER_ERROR", "internal server error"))))
    ...
    "return a 503 error response with a calculation error" in {
      ...
      mockAuth()
      calculationResponseByCalcId(nino, calculationId, Some(taxYear), None)(Left(ErrorModel(SERVICE_UNAVAILABLE, ErrorBodyModel("SERVICE_UNAVAILABLE", "service unavailable"))))
    ...
    "return a 400 error response with a calculation error" in {
      ...
      mockAuth()
      calculationResponseByCalcId(nino, calculationId, Some(taxYear), None)(Left(ErrorModel(BAD_REQUEST, ErrorBodyModel("BAD_REQUEST", "bad request"))))
    ...
    "return a 403 error response with a calculation error" in {
      ...
      mockAuth()
      calculationResponseByCalcId(nino, calculationId, Some(taxYear), None)(Left(ErrorModel(FORBIDDEN, ErrorBodyModel("FORBIDDEN", "forbidden"))))
